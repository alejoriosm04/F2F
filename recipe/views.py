    ...
    if form.is_valid():
        ...
        if not user_ingredients:
            return render(
                request,
                "recipe.html",
                {"recipe": None, "error_message": "User has no ingredients."},
            )
        ...
        recipe = adapter.generate_response_sync(
            user_ingredients, details, preference, portions
        )
        ...
        return render(
            request, "recipe.html", {"recipe": recipe, "error_message": error_message}
        )
...
class RecipeHistoryView(ListView):
    ...
    def get_queryset(self):
        return Recipe.objects.filter(user=self.request.user).order_by(
            "-generation_date"
        )  # Recent recipes first.
