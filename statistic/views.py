...
def generate_recommendation(request):
    ...
    if last_report and (now().date() - last_report.report_date).days < 15:
        ...
        return redirect('statistics_overview')  # Redirect to overview or an error message page

    # Gather user data for the recommendation
    user_ingredients = Ingredient.objects.filter(user=user).values_list('name', flat=True)
    favorite_recipes = Recipe.objects.filter(user=user, favourite_state=True).values_list('title', flat=True)
    ...
    if isinstance(recommendation_json, str):
        recommendation_info = json.loads(recommendation_json)  # Parse JSON string into Python dictionary
    else:
    ...
    new_report = Report(user=user, report_date = now(), report_info=json.dumps(recommendation_info))
    new_report.save()
